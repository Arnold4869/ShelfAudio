...
/** 该书的本地音轨映射：{ [idx]: uri } */
export async function localTrackMap(bookId) {
  const out = {}
  for (const k of Object.keys((await offlineInfo(bookId))?.tracks || {})) {
    const uri = await localTrackUri(bookId, Number(k))
    if (uri) out[Number(k)] = uri
  }
  return out
}
